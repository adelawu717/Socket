...
class DynamicPlotApp:
    ...
    def process_received_file(self):
        try:
            ...
            predicted_probabilities = clf.predict_proba(new_input)

            # Print or save the predictions
            print(f"Classification result: Motion{predictions+1}")
            print("Predicted probabilities:", predicted_probabilities)
            class_possibility = predicted_probabilities[0, predictions]
            print("Class probabilities:", class_possibility)
            from gradingSystem import calculate_score
